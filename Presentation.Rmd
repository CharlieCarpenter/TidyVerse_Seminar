...
## Plotting with `ggplot2`
  ...
  One of the first entries in the tidyverse was the `ggplot2` package. There are many methods of creating plots within R, but `ggplot2` claims to be "one of the most elegant and most versatile" packages. I tend to whole-heartedly agree with this claim :). 
...
Above, we can see th specification of `geom_smooth` as opposed to `geom_point` alters the plotting layer from the previously seen scatter plot to a smoothing line instead. 
...
* Interested in learning more about `ggplot2`? Here are the [Top 50 ggplot2 Visualizations - The Master List (With Full R Code)](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html)
