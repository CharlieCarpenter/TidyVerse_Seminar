...
## Summary
  ...
  Again, there is nothing you can do in the tidyverse that you can't do using base R functions. The exception being some of the visualizations from `ggplot` and its extensions.
  
  At worst, the tidyverse seems like a reworking of `base` R functions so that they don't have the small things from `base` that annoyed Hadley. At best, it is large improvements over `base` functions like `ggplot` and `%>%`. That said, there are plenty of people who are against this new coding style. 
